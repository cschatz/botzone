...
while ( ($item, $prompt, $type, $multi, $hasgroup, $seq) = $outer->fetchrow_array())
{
...
EOF
	...
	elsif ($subtype eq "M")
	{
		...
		print "<tr><td><input class='box' type='$what' name='$item' value='$code'></td><td>($code)$answer</td></tr>";
	    }
	    print "</table>\n";
